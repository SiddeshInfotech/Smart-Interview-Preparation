...
PROJECT_DOWNLOADS_DIR = os.path.join(BASE_DIR, "..", "downloads", "HTML_and_CSS_Topic_PDFs")
USER_DOWNLOADS_DIR = r"C:\Users\Admin\Downloads\HTML_and_CSS_Topic_PDFs"
...
if os.path.exists(PROJECT_DOWNLOADS_DIR) and os.listdir(PROJECT_DOWNLOADS_DIR):
    ...
    raise FileNotFoundError(f"HTML & CSS PDFs directory not found in '{PROJECT_DOWNLOADS_DIR}' or '{USER_DOWNLOADS_DIR}'.")
...
# Adjust existing React course sequence if needed so HTML & CSS appears first
Course.objects.filter(domain=domain, technology="React").update(sequence=2)

# 2. Get or create HTML & CSS Course under Web Development
...
print(f"Successfully imported {len(pdf_files)} HTML & CSS topic PDFs into Course: '{course.title}' under Domain: '{domain.name}'!")
