...
def get_optimized_daily_progress(user):
    """
    Computes 7-day cumulative performance for Quiz, Coding, and Interview.
    Optimized: Uses only 3 bulk database queries total instead of 21 loop queries.
    """
    if not user or not user.is_authenticated:
        return [
            {"day": (timezone.now().date() - timedelta(days=i)).strftime("%a"), "date": (timezone.now().date() - timedelta(days=i)).strftime("%b %d"), "quiz": 0, "coding": 0, "interview": 0}
    ...
    try:
        ...
        for i in range(6, -1, -1):
            ...
            # 1. Cumulative Quiz Avg in memory
            ...
            # 2. Cumulative Coding Avg in memory
            ...
            # 3. Cumulative Interview Rating in memory
