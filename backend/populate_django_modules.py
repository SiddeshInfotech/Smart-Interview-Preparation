...
PROJECT_DOWNLOADS_DIR = os.path.join(BASE_DIR, "..", "downloads", "Django_Notes_Topic_PDFs")
USER_DOWNLOADS_DIR = r"C:\Users\Admin\Downloads\DJango"
...
if os.path.exists(PROJECT_DOWNLOADS_DIR) and os.listdir(PROJECT_DOWNLOADS_DIR):
    ...
    raise FileNotFoundError(f"Django PDFs directory not found in '{PROJECT_DOWNLOADS_DIR}' or '{USER_DOWNLOADS_DIR}'.")

print(f"Using PDF source directory: {SPLIT_DIR}")

# 1. Get or create Web Development domain
domain, _ = Domain.objects.get_or_create(
    name="Web Development",
    defaults={"description": "Full Stack Web Development & Frontend/Backend Frameworks"}
)

# Ensure sequence order in Web Development: HTML/CSS (1), React (2), Python (3), Django (4)
...
Course.objects.filter(domain=domain, technology="Python").update(sequence=3)

# 2. Get or create Django Course under Web Development domain
...
print(f"Successfully imported {len(pdf_files)} Django topic PDFs into Course: '{course.title}' under Domain: '{domain.name}'!")
